...
class EditGuestFragment : Fragment {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val guestName = view.findViewById<EditText>(R.id.edit_guest_name_edit)
        val guestBirthday = view.findViewById<MaskEditText>(R.id.edit_guest_birthday_edit)
        val guestRoom = view.findViewById<EditText>(R.id.edit_guest_room_edit)

        guestBirthday.setText(guest!!.birthday.format(DateTimeFormatter.ofPattern("dd.MM.yyyy")))
        guestRoom.setText(guest!!.room.number.toString())
        guestName.setText(guest!!.name)

        view.findViewById<Button>(R.id.edit_guest_save_button).setOnClickListener {
            try {
                ...
                dataStorage.replaceGuest(Guest(
                    guest!!.id,
                    name,
                    settedDate,
                    Room(roomNumber)
        ...
        view.findViewById<Button>(R.id.edit_guest_back_button).setOnClickListener {
            MainActivity.instance?.supportFragmentManager?.commit {
                setCustomAnimations(R.anim.open_animator, R.anim.close_animator)
                replace(R.id.fragmentContainerView, GuestsFragment(), "guests")
                addToBackStack(null)
